...
useEffect(() => { localStorage.setItem('relics', JSON.stringify(relics)); }, [relics]);

function gainXP(amount, reason) { setXP(prev => prev + amount); setVaultFeed(prev => [...prev, +${amount} XP: ${reason}]); setToasts(prev => [...prev, +${amount} XP!]); if (xp + amount >= 100 && !achievements.includes('First Level Up')) { setAchievements(prev => [...prev, 'First Level Up']); setToasts(prev => [...prev, 'Achievement: First Level Up!']); } }

function inviteFriend(friend) { setPartyInvites(prev => [...prev, friend]); setVaultFeed(prev => [...prev, Party invite sent to ${friend}]); }

function unlockRelic(name) { setRelics(prev => [...prev, name]); setVaultFeed(prev => [...prev, Relic equipped: ${name}]); }
